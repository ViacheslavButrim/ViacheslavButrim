...
for (let i = 0; i < NUM_STARS; i++) {
  ...
  svg += `
    <circle class="star" cx="${x}" cy="${y}" r="${size}" fill="${color}">
      <animate attributeName="r" values="${size};${size*2};${size}" dur="${pulse}s" begin="${delay}s" repeatCount="indefinite"/>
      <animate attributeName="opacity" values="0.2;1;0.2" dur="${pulse}s" begin="${delay}s" repeatCount="indefinite"/>
      <animate attributeName="cx" values="${x};${x+random(-2,2)};${x}" dur="${pulse*2}s" begin="${delay}s" repeatCount="indefinite"/>
      <animate attributeName="cy" values="${y};${y+random(-2,2)};${y}" dur="${pulse*2}s" begin="${delay}s" repeatCount="indefinite"/>
    </circle>
  `;
}
  ...
  svg += `
    <circle r="${size}" fill="hsl(${random(190,220)}, 100%, 70%)">
      <animateMotion dur="${speed}s" repeatCount="indefinite" begin="${delay}s">
        <mpath>
          <path d="M ${orbitCx-orbitRadius},${orbitCy} a ${orbitRadius},${orbitRadius} 0 1,1 ${orbitRadius*2},0 a ${orbitRadius},${orbitRadius} 0 1,1 -${orbitRadius*2},0"/>
        </mpath>
      </animateMotion>
